...
(defthmd numeric-pigeon-hole-property
  (implies
   (and
    (natp m)
    (< (n) m))
...
(defmacro hole-proof (fn n &key (function 'nil))
  (let ((pkg (or function (if (symbolp fn) fn 'hole-proof)))
    ...
    (let ((exists-fn-duplicates         (acl2::packn-pos (list 'exists- function '-duplicates) pkg))
          ...
          (fn-pigeon-hole-property      (acl2::packn-pos (list function '-pigeon-hole-property) pkg)))
      `(encapsulate
           ()
         ...
         (defthm ,fn-pigeon-hole-property
           (implies
            (and
             (natp m)
             (< ,n m))
           ...
           :hints (("Goal" :do-not-induct t
                    :use (:functional-instance numeric-pigeon-hole-property
                                               (fn ,fn)
                                               (n  (lambda () ,n))
                   ...
                   (and stable-under-simplificationp
                        '(:in-theory (disable
                                      ,exists-fn-duplicates-suff
                                      )
                                     :use (
                                           #+joe
                                           (:instance ,exists-fn-duplicates-suff
                                                      (a (mv-nth 0 (exists-duplicates-witness m)))
                                                      (b (mv-nth 1 (exists-duplicates-witness m))))
                                           #+joe
                                           (:instance exists-duplicates-suff
                                                      (a (mv-nth 0 (,exists-fn-duplicates-witness m)))
                                                      (b (mv-nth 1 (,exists-fn-duplicates-witness m))))
                                           )))))
